<template>
  <div class="mt-5 flex align-items-center m-2">
    ...
    <div style="font-size: 2rem" class="mr-2">{{userData.username}}</div>
    <div v-if="showLink">
      <Button label="Аккаунт" size="small" link @click="$router.push('/account/'+userData.username)"/>
    </div>
  </div>

  <div class="my-3 text-500 flex flex-row flex-wrap">
    <div v-if="userData.email" class="mx-2 flex align-items-center my-2">
    ...
    <div v-if="userData.registration_date" class="mx-2 flex align-items-center">
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="mr-2" viewBox="0 0 16 16">
...
export default {
  ...
  props: {
    ...
    showLink: {required: false, type: Boolean, default: false},
  },
